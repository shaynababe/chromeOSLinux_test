...
PREFIX='/usr/local/'
PREFIXSET=''
CHROOTSLINK='/mnt/stateful_partition/chromeOSLinux/chroots'
PROXY='unspecified'
RELEASE=''
RESTORE=''
RESTOREBIN=''
DEFAULTRELEASE='bullseye'
PREVIOUS_DEFAULT_RELEASES='focal'
TARBALL=''
TARGETS=''
TARGETFILE=''
UPDATE=''
...
if [ -z "$DOWNLOADONLY" -a -n "$TARBALL" ]; then
    ...
    if [ -n "$label" ]; then
        ...
        elif [ "${label#chromeOSLinux:bootstrap}" != "$label" ]; then
            releasearch="${label#*.}"
        else
            error 2 "$TARBALL doesn't appear to be a valid chromeOSLinux bootstrap."
        fi
    else
...
if [ -z "$RESTOREBIN$DOWNLOADONLY" ]; then
    ...
    # symbolic link to /mnt/stateful_partition/chromeOSLinux/chroots ($CHROOTSLINK)
...
You also have to specify a mirror to chromeOSLinux (-m) for installation to proceed."
        fi
        echo "\
...
# successful.
vboot_is_safe() {
    local tmp="`mktemp -d --tmpdir=/usr/local/bin 'chromeOSLinux-rwtest.XXX'`"
...
#       mount -i -t tmpfs -o 'rw,dev,exec' tmpfs "$tmp"
    ...
    if [ -n "$DOWNLOADONLY" ]; then
        ...
        $FAKEROOT tar -C "$tmp" -V "chromeOSLinux:bootstrap.$subdir" \
                      -cajf "$TARBALL" "$subdir"
        echo 'Done!' 1>&2
        exit 0
    fi
...
if [ -z "$RESTOREBIN" ]; then
    # Ensure that /usr/local/bin and /etc/chromeOSLinux exist
    mkdir -p "$CHROOT/usr/local/bin" "$CHROOT/etc/chromeOSLinux"

    # If -U was not specified, update existing targets.
    if [ -z "$UPDATEIGNOREEXISTING" ]; then
        TARGETSFILE="$CHROOT/etc/chromeOSLinux/targets"
...
Please specify targets with -t."
    ...
    for chroot in "$CHROOTS"/*; do
        deduptargets "$chroot/.chromeOSLinux-targets"
    done
fi
...
if [ -f "$PREPARE" ]; then
    # Update .chromeOSLinux-targets in the unencrypted part of the chroot
    cp -fT "$TARGETDEDUPFILE" "$CHROOTSRC/.chromeOSLinux-targets"
