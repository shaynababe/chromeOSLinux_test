...
# Takes in a list of chromeOSLinux-style package names, and outputs the list, filtered
# for the current distro.
# A chromeOSLinux-style package name looks something like the following:
...
# For the specified chromeOSLinux-style package names (see distropkgs()), installs
...
# parameters are a list of chromeOSLinux-style package names to "install", optionally
...
# list_uninstalled: For the specified chromeOSLinux-style package names (see
...
# assuming they are consoles. This isn't true for Chromium OS and chromeOSLinux, and
...
# $1: name; target is /usr/local/(bin|lib)/chromeOSLinux$1[.so]
# $2: linker flags, quoted together
# [$3]: if 'so', compiles as a shared object and installs it into lib
# $3+: any package dependencies other than gcc and libc-dev, chromeOSLinux-style.
compile() {
    local out="/usr/local/bin/chromeOSLinux$1"
    local linker="$2"
    local cflags='-xc -Os -I/usr/src/chromeOSLinux'
    if [ "$3" = 'so' ]; then
        out="/usr/local/lib/chromeOSLinux$1.so"
