...
SRCTARGETS = $(patsubst src/%.c,chromeOSLinux%,$(filter-out $(LIBS),$(wildcard src/*.c)))
...
chromeOSLinuxfbserver_LIBS = -lX11 -lXdamage -lXext -lXfixes -lXtst
chromeOSLinuxxi2event_LIBS = -lX11 -lXi
chromeOSLinuxfreon.so_LIBS = -ldl -ldrm -I/usr/include/libdrm

chromeOSLinuxwebsocket_DEPS = src/websocket.h
chromeOSLinuxfbserver_DEPS = src/websocket.h
...
$(SRCTARGETS): $(patsubst chromeOSLinux%,src/%.c,$@) $($@_DEPS) Makefile
	gcc $(CFLAGS) $(patsubst chromeOSLinux%,src/%.c,$@) $($@_LIBS) -o $@

$(LIBSTARGETS): $(patsubst chromeOSLinux%.so,src/%.c,$@) $($@_DEPS) Makefile
	gcc $(CFLAGS) -shared -fPIC $(patsubst chromeOSLinux%.so,src/%.c,$@) $($@_LIBS) -o $@
...
clean:
	...
	rm -rf $(BUILDDIR)
	rm -rf /usr/local/bin/debootstrap
