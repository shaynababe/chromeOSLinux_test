...
function refreshUI() {
    ...
    chrome.browserAction.setTitle({title: 'chromeOSLinux: ' + icon});
...
function clipboardStart() {
    ...
    /* Monitor window/tab focus changes/removals and report to chromeOSLinuxclip */
...
function websocketMessage(evt) {
    ...
    case 'N': /* Raise a notification */
        ...
        try {
            ...
            /* Strip off chromeOSLinux fields */
            var id = "";
            var display = null;

            if (data.chromeOSLinux_id) {
                id = data.chromeOSLinux_id;
            }
            delete data.chromeOSLinux_id;

            /* Set context message with chroot name/display */
            delete data.contextMessage;
            if (data.chromeOSLinux_display) {
                display = data.chromeOSLinux_display;
            ...
            delete data.chromeOSLinux_display;
        ...
        break;
    case 'C': /* Returned data from a chromeOSLinuxcycle command */
        ...
        refreshUI();
        break;
    case 'X': /* Ask to open a chromeOSLinux window */
        var display = payload;
        ...
        if (!displaynum) {
            ...
            var name = win ? win.name : "chromeOSLinux in a window";
            var create = chrome.windows.create;
            var data = {};

            if (kiwi_win_[display].isTab) {
                name = win ? win.name : "chromeOSLinux in a tab";
...
chrome.runtime.getPlatformInfo(function(platforminfo) {
    if (platforminfo.os == 'cros') {
        ...
        chrome.browserAction.setTitle(
            {title: 'chromeOSLinux is not available on this platform'}
        );
